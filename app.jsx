  ...
  return (
    <div className={"app theme-" + t.theme} data-screen-label="Sequencer">
      ...
      <div className="main">
        ...
        <div className="right">
          <PreviewStage balls={balls} litState={litState} layout={t.previewLayout} glow={t.ballGlow} />
          <Inspector
            step={selectedStep}
            updateStep={updateStep}
...
// ============ PREVIEW ============
function PreviewStage({ balls, litState, layout, glow }) {
  return (
      ...
      <div className={"preview-stage layout-"+layout}>
        {balls.map((b, i) => (
          <BallPreview key={b.id} ball={b}
            ...
            glow={glow} index={i} total={balls.length} layout={layout}
          />
        ))}
      </div>
    </div>
...
function BallPreview({ ball, ledA, ledB, glow, index, total, layout }) {
  const aOn = ledA && ledA.brightness > 0;
  const bOn = ledB && ledB.brightness > 0;
  const anyOn = aOn || bOn;

  // Real device has two discrete RGB LEDs at separate positions inside a
  // translucent diffuser ball. They don't blend like a 50/50 paint gradient —
  // each is a point source whose light falls off with distance, and where they
  // overlap the colors ADD (R+R, G+G, B+B clamped to 255).
  // We model that with two radial gradients (one per LED) using `screen` mix-
  // blend-mode so the overlap is additive, plus per-LED outer halos.

  // LED A sits on the upper-left hemisphere, LED B on the lower-right —
  // matches a typical 2-LED ball layout where the LEDs are on opposite poles.
  const A_POS = { x: 32, y: 28 }; // % within orb
  const B_POS = { x: 68, y: 72 };

  const aColor = aOn ? ledA.color : '#000';
  const bColor = bOn ? ledB.color : '#000';
  const aBri = aOn ? ledA.brightness : 0;
  const bBri = bOn ? ledB.brightness : 0;

  // Inner light zones — additive via mix-blend-mode: screen.
  // Each LED's reach: bright core ~30% radius, soft falloff to 75%.
  const ledZoneA = aOn ? `radial-gradient(circle at ${A_POS.x}% ${A_POS.y}%,
      ${rgbaWithAlpha(aColor, 0.95 * aBri)} 0%,
      ${rgbaWithAlpha(aColor, 0.55 * aBri)} 22%,
      ${rgbaWithAlpha(aColor, 0.18 * aBri)} 50%,
      transparent 78%)` : 'transparent';
  const ledZoneB = bOn ? `radial-gradient(circle at ${B_POS.x}% ${B_POS.y}%,
      ${rgbaWithAlpha(bColor, 0.95 * bBri)} 0%,
      ${rgbaWithAlpha(bColor, 0.55 * bBri)} 22%,
      ${rgbaWithAlpha(bColor, 0.18 * bBri)} 50%,
      transparent 78%)` : 'transparent';
  ...
  // Outer halo — light leaking out of the diffuser ball.
  // Each LED's halo is anchored at its position via box-shadow with offset.
  // 22 = ball radius in px; offsets push the shadow center toward each LED.
  const orbR = 22;
  const aOff = { x: (A_POS.x - 50) / 50 * orbR, y: (A_POS.y - 50) / 50 * orbR };
  const bOff = { x: (B_POS.x - 50) / 50 * orbR, y: (B_POS.y - 50) / 50 * orbR };
  const haloA = aOn ? `${aOff.x}px ${aOff.y}px ${14 + aBri * 22}px ${4 + aBri * 4}px ${rgbaWithAlpha(aColor, 0.55 * aBri * glow)}` : '';
  const haloB = bOn ? `${bOff.x}px ${bOff.y}px ${14 + bBri * 22}px ${4 + bBri * 4}px ${rgbaWithAlpha(bColor, 0.55 * bBri * glow)}` : '';
  const halo = [haloA, haloB].filter(Boolean).join(', ');
  ...
  return (
    <div className="ball-prev" style={style}>
      <div className="ball-orb" style={{
        ...
        {/* additive LED light layers */}
        <div className="ball-light" style={{ background: ledZoneA, mixBlendMode: 'screen' }}/>
        <div className="ball-light" style={{ background: ledZoneB, mixBlendMode: 'screen' }}/>
        {/* specular highlight over the top */}
        <div className="ball-spec"/>
        {/* tiny LED dies visible through the diffuser */}
        {aOn && (
          <div className="ball-die" style={{
            left: A_POS.x + '%', top: A_POS.y + '%',
            background: aColor, boxShadow: `0 0 4px ${aColor}`, opacity: aBri,
          }}/>
        )}
        {bOn && (
          <div className="ball-die" style={{
            left: B_POS.x + '%', top: B_POS.y + '%',
            background: bColor, boxShadow: `0 0 4px ${bColor}`, opacity: bBri,
          }}/>
        )}
      </div>
...
// Convert hex color + alpha → rgba() string, for layered light gradients.
function rgbaWithAlpha(hex, a) {
